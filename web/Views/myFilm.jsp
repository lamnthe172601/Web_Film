                    ...
                    <div id="myTabContent" class="tab-content">
                        <div role="tabpanel" class="tab-pane fade in active" id="home" aria-labelledby="home-tab">
                            <div class="agile-news-table">
                                <form action="MyFilmController" method="post">
                                    <div class="w3ls-news-result" style="float: right">
                                        <input type="hidden" name="mode" value="1"/>
                                        <button type="submit" class="btn btn-info" data-toggle="modal" data-target="#popupForm">
                                            Delete All
                                        </button>                                    
                                    </div>
                                </form>
                                <table id="table-breakpoint">
                                    ...
                                    <tbody>
                                        <c:forEach items="${filmcountrygenrelist}" var="filmcountrygenre" varStatus="loop">  
                                            <tr>
                                                <td>${loop.index+1}</td>
                                                <c:if test="${filmcountrygenre.getFilm().isIsActive()}" >
                                                    <td class="w3-list-img"><a href="FilmDetailController?id=${filmcountrygenre.getFilm().filmId}"><img src="${filmcountrygenre.getFilm().linkImg2}" alt="" /> <span>${filmcountrygenre.getFilm().getFilmName()}</span></a></td>
                                                    <c:if test="${filmcountrygenre.getEpisode().isActive()}">
                                                    <td><a href="EpisodeController?id=${filmcountrygenre.getFilm().filmId}&episodeid=${filmcountrygenre.getEpisode().getEpisodeid()}">${filmcountrygenre.getEpisode().getName()}</a></td>
                                                   </c:if>
                                                    <c:if test="${!filmcountrygenre.getEpisode().isActive()}">
                                                    <td><a>${filmcountrygenre.getEpisode().getName()}</a></td>
                                                   </c:if>
                                                    </c:if>
                                                    <c:if test="${!filmcountrygenre.getFilm().isIsActive()}" >

                                                    <td class="w3-list-img"><a><img src="${filmcountrygenre.getFilm().linkImg2}" alt="" /> <span>${filmcountrygenre.getFilm().getFilmName()}</span><span style="color: red">movie is not available</span></a></td>
                                                    <td><a>${filmcountrygenre.getEpisode().getName()}</a></td>
                                                    </c:if>
                                                <td>${filmcountrygenre.getUserview().getDate()}</td>
                                                <c:if test="${filmcountrygenre.getFilm().isSeries}">
                                                    <td><a href="SeriesController?index=1">TV-Series</a></td>
                                                </c:if>
                                                <c:if test="${!filmcountrygenre.getFilm().isSeries}">
                                                    <td><a href="SingleController?index=1">Single Film</a></td>
                                                </c:if>
                                                <td class="w3-list-info"><a href="CountryController?id=${filmcountrygenre.getCountry().countryId}&index=1">${filmcountrygenre.getCountry().countryName}</a></td>
                                                <td class="w3-list-info">
                                                    <c:forEach items="${filmcountrygenre.getGenre()}" var="o">
                                                        <a href="GenreController?id=${o.genreId}&index=1">
                                                            ${o.genreName}
                                                        </a>
                                                    </c:forEach>
                                                </td>
                                                <td>
                                                    <form action="MyFilmController" method="post">
                                                        <div class="w3ls-news-result" style="float: right">
                                                            <input type="hidden" name="userviewid" value="${filmcountrygenre.getUserview().getUserViewId()}"/>
                                                            <button type="submit" class="btn btn-info" data-toggle="modal" data-target="#popupForm">
                                                                Delete
                                                            </button>                                    
                                                        </div>
